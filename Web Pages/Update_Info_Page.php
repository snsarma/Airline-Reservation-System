	...
	<script type = "text/javascript">
			...
			<!--
				...
				function validation()
				{
					...
					else if(u_firstname.value.length != 0 && !u_firstname.value.match(name_pattern))
					{
						error_message_area.innerHTML = "The first name of the customer you specify needs to be valid";
						return false;
					}
					
					else if(u_ssn.value.length != 0 && !u_ssn.value.match(ssn_pattern))
					{
						error_message_area.innerHTML = "The SSN of the customer you specify needs be valid";
						return false;
					}
					...
					else if((firstname.value.length != 0) && !(firstname.value.match(name_pattern)))
					{
					...
					else if((lastname.value.length != 0) && !(lastname.value.match(name_pattern)))
					{
						error_message_area.innerHTML = "Incorrect name !<br> You've included non-valid characters as part of your name.<br> Please use valid characters when writing your name." ;
						return false;
					}
					else if((firstname.value.length != 0 && lastname.value.length != 0)&& (firstname.value == lastname.value))
					{
						error_message_area.innerHTML = "First name and last name identical !<br>User should have different first and last name(s)."
						return false;
					}

					else if ((email.value.length != 0) && (!email.value.match(email_pattern)))
					{
						error_message_area.innerHTML = "Invalid e-mail !<br> Enter a valid email like abc@yahoo.com";
						return false;
					}
					
					else if((ssn.value.length != 0) &&(!ssn.value.match(ssn_pattern))){

						error_message_area.innerHTML = "You entered an invalid SSN !";
						return false;
					}
					
					else if((area_code.value.length != 0) &&(!area_code.value.match(area_code_pattern))){
						
						error_message_area.innerHTML = "The area code should consist of three digits!";
						return false;
					}
					
					else if((second_part.value.length != 0) &&(!second_part.value.match(second_part_pattern))){
						
						error_message_area.innerHTML = "The second part of your phone number should have three digits!";
						return false;
					}
					
					else if((last_part.value.length != 0) &&(!last_part.value.match(last_part_pattern))){
		...
		var phpScriptURL = "http://localhost/Website/Scripting/Update_Script.php";
		...
		"&last_part="+last_part.value+"&u_ssn="+u_ssn+"&u_firstname="+u_firstname.value;
...
//-->
	...
	<li> <a href = "http://localhost/Website/Scripting/Customer_Search_Page.php"> Search Customer</a></li>
	<li> <a href = "http://localhost/Website/Scripting/Customers_List.php"> All Customers </a></li>
	<li> <a href = "http://localhost/Website/Web Pages/Update_Info_Page.html"> Update customer info </a></li>
	...
	<table style = "position:absolute; margin-top:250px; margin-left : 50px"
				...
				<tr> <td> <label id = 'label'> SSN </label> </td><td> <input id = 'u_ssn' maxlength = '15' class = 'input'  style = 'border-radius:5px;'/> </td></tr>
			    <tr> <td> <label id = 'label'> First name </label> </td><td> <input id = 'u_firstname' class ='input'   style = 'border-radius:5px;'/> </td></tr>
	</table>
